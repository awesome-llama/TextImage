...
class ChunkA8(lu.Chunk):
    ID = 'A8' # the identifying string for this chunk
...
def compress(image_array:list, dimensions:tuple, lossy_tolerance=0, RLE=True, debug=False):
    ...
    return lu.DataStream(''.join(string_chunks), ChunkA8.ID, '0')
...
def decompress(stream:str, dimensions:tuple, debug=False):
    ...
    def _process_chunk(chunk):
        match chunk.name[0]:
            case 'raw':
                _add_val((chunk.name[1]*94 + chunk.data[0]))

            case 'copy_prev':
                _add_val(col_prev[0])

            case 'copy_vert_fwd':
                _add_val(col_prev[dimensions[0] - 2])

            case 'copy_vert':
                _add_val(col_prev[dimensions[0] - 1])

            case 'copy_vert_back':
                _add_val(col_prev[dimensions[0] + 0])
    ...
    i = 0
    while i < len(stream):
        op_name = ChunkA8.get_op_name(stream[i])

        if op_name[0] == 'repeat_op':
            repeat_op_name = ChunkA8.get_op_name(stream[i+1]) # the op that should be repeated
            repeat = 2 + stream[i+2] * ChunkA8.get_op_size(repeat_op_name)
        else:
            repeat = ChunkA8.get_op_size(op_name)
    ...
    for chunk in chunks:
        ...
        if chunk.name[0] == 'repeat_op':
            op_name = ChunkA8.get_op_name(chunk.data[0])
            op_size = ChunkA8.get_op_size(op_name)
            repeat = chunk.data[1]
